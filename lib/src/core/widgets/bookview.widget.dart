      ...
      listener: (context, state) async {
        ...
        if (state is NavigateSucessfull) {
          if (kIsWeb) {
            ...
            await GoRouter.of(context).pushNamed(RoutesNameEnum.book.name,
                pathParameters: {"title": state.book.title}, extra: state.book)
                .then((_) =>   context.read<DashboardBloc>().add(ContinueReadingEvent()));
          }
        }
      },
      child: Padding(
        ...
        child: InkWell(
          onTap: () {
             context.read<DashboardBloc>().add(NavigatePageEvent(book: book!));
